...
(async () => {
  try {
    const url = process.argv[2] || 'http://localhost:5173/billing-payment-management';
    ...
    const resp = await page.goto(url, { waitUntil: 'networkidle2', timeout: 20000 }).catch(e => { console.error('GOTO ERROR', e.message); process.exit(2); });
